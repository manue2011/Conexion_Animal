...
const MetricasPage = () => {
  ...
  // --- NUEVOS ESTADOS PARA EL MODAL PRO ---
  ...
  // --- FUNCIÓN PARA CARGAR LA LISTA DE USUARIOS PRO ---
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen space-y-10 animate-fade-in">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
          ...
          <div 
            ...
            className="bg-gradient-to-br from-yellow-50 to-white p-6 rounded-xl shadow-sm border-l-4 border-yellow-500 relative overflow-hidden cursor-pointer hover:shadow-md hover:scale-105 transition-all group"
          >
            <p className="text-xs font-bold text-yellow-600 uppercase flex items-center justify-between">
              <span className="flex items-center gap-1">👑 Planes PRO</span>
              <span className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px]">VER LISTA ➔</span>
            </p>
            <p className="text-3xl font-black mt-2 text-gray-800">{counts.suscripciones_pro}</p>
            {counts.suscripciones_pro > 0 && (
              <span className="text-[10px] text-yellow-600 font-bold mt-1 tracking-tighter block">
                INGRESOS ACTIVOS
              </span>
            )}
            <div className="absolute -right-4 -bottom-6 opacity-10 text-yellow-500 text-7xl font-black group-hover:scale-110 transition-transform">
              ★
            </div>
          </div>
        </div>
      ...
      {/* --- MODAL FLOTANTE CON LA LISTA DE USUARIOS PRO --- */}
      {isProModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border-t-8 border-yellow-500 flex flex-col max-h-[80vh]">
            
            {/* Header del Modal */}
            <div className="p-6 border-b flex justify-between items-center bg-yellow-50/50 rounded-t-xl">
              <div>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {loadingProUsers ? (
                ...
                <div className="space-y-3">
                  {proUsers.map((user) => (
                    <div key={user.id} className="flex items-center justify-between p-4 bg-gray-50 border border-gray-100 rounded-xl hover:border-yellow-300 transition-colors">
